...
func checkProtection(w http.ResponseWriter, r *http.Request, req *Request, config *cf.Configuration, protected RouteProtection) error {
	...
	if protected == RouteProtectionHMAC {
		presented, hmacErr := checkHmacPresent(req, config.PayloadSecret)
		if hmacErr != nil {
...
func parseCookie(w http.ResponseWriter, cookieMap map[string]*http.Cookie, config *cf.Configuration) (jwt.MapClaims, error) {
	...
	publicKey, keyErr := jwt.ParseECPublicKeyFromPEM([]byte(*config.PublicKey))
	if keyErr != nil {
		writeError("Internal Error happened in reading cookie!",
...
// validateRequest
func validateRequest(req *Request, payloadSecret *string) (err error) {
...
func checkHmacPresent(req *Request, payloadSecret *string) (bool, error) {
	...
	if xCloudSignature != "" {
		validErr := validateRequest(req, payloadSecret)
		if validErr != nil {
